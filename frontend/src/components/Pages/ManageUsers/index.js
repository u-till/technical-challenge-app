...
const ManageUsers = ({allUsers, getAllUsersAction, notEmpty}) => {
    ...
    const renderUsers = (searchedUsers) => {
        const mapUserCard = (arr) => {
            return arr.map((user) => <GenericUserCard key={`User ${user.id}`} user={user}/>);
        };
        switch (filterRole) {
            case ("all"): {
                if (sort === "last_name") {
            ...
            case ("staff"): {
                if (sort === "last_name") {
                    return mapUserCard(sortByLastName(filterByStaff(searchedUsers, true)))
                } else {
                    return mapUserCard(sortByLastName(filterByStaff(searchedUsers, true)))
                }
            }
            default:
                if (sort === "last_name") {
                    return mapUserCard(sortByLastName(filterByStaff(searchedUsers, false)))
                } else {
