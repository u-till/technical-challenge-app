...
export const confirmPasswordResetAction = (resetData) => async (dispatch) => {
    try {
        const response = await Axios.patch('users/password/reset/validation/', resetData);
        return response
    } catch (error) {
        if (error.response.data.detail === "Your email doesn't match any profile or is invalid.") {
            dispatch(nonFieldVerificationError(error.response.data.detail));
        } else {
            let errors = {};
            for (let i of Object.keys(error.response.data)) {
